...
function get_ua_info($ua_string, $detailed=1)
{
	...
	elseif(eregi('Iceweasel', $ua_string))
	{	// Version de développement de Firefox
		$ua['name'] = 'Iceweasel';
		$ua['ua_class'] = 'Firefox';
		$ua['ua_type'] = 'Browser';
		if($detailed)
		{
			$ua['ua_version'] = get_ua_version($ua_string, 'Iceweasel');
		}//detailed
	}
	elseif(ereg('Shiretoko', $ua_string))
	{	// Version de développement de Firefox
		$ua['name'] = 'Shiretoko';
		$ua['ua_class'] = 'Firefox';
		$ua['ua_type'] = 'Browser';
		if($detailed)
		{
			$ua['ua_version'] = get_ua_version($ua_string, 'Shiretoko');
		}//detailed
	}
	elseif(ereg('Minefield', $ua_string))
	{	// Version de développement de Firefox
		$ua['name'] = 'Minefield';
		$ua['ua_class'] = 'Firefox';
		$ua['ua_type'] = 'Browser';
		if($detailed)
		{
			$ua['ua_version'] = get_ua_version($ua_string, 'Minefield');
		}//detailed
	}
	...
	elseif (ereg('Firefox', $ua_string))
	{
		...
		$ua['ua_type'] = 'Browser';
			if($detailed)
			{
			$ua['ua_version'] = '(basé sur Firefox '.get_ua_version($ua_string, 'Firefox').')';
			}//detailed
		}
		else{
		$ua['name'] = 'Firefox';
	...
	{	// la chaîne AU d'Omniweb peut contenir les mots Safari, KHTML et Gecko, il faut donc le détecter avant,
		...
		if($detailed)
		{
			if(eregi('OmniWeb/v5.9.1', $ua_string)) $ua['ua_version'] = '5.9.1';
			elseif(eregi('OmniWeb/v5', $ua_string)) $ua['ua_version'] = '5';
	...
	elseif(ereg('Chrome', $ua_string))
	{	// la chaîne AU de Chrome peut contenir les mots Safari, KHTML et Gecko, il faut donc le détecter avant,
		// bien que ce soit un navigateur rare. Déjà rencontré : Shiira/1.2, Shiira/0.9.5.1
		$ua['name'] = 'Chrome';
		$ua['ua_class'] = 'Chrome';
		$ua['ua_type'] = 'Browser';
		if($detailed)
		{
			$ua['ua_version'] = get_ua_version($ua_string, 'Chrome');
		}//detailed
	}
	elseif(ereg('iPhone', $ua_string))
	{	// la chaîne AU de l'Iphone
		$ua['name'] = 'Safari Mobile';
		$ua['ua_class'] = 'Safari';
		$ua['ua_type'] = 'MobileDevice';
		if($detailed)
		{
			$ua['ua_version'] = get_ua_version($ua_string, 'Version');
		}//detailed
	}
	elseif(ereg('iPod', $ua_string))
	{	// la chaîne AU de l'Ipod Touch
		$ua['name'] = 'Safari Mobile';
		$ua['ua_class'] = 'Safari';
		$ua['ua_type'] = 'MobileDevice';
		if($detailed)
	...
	elseif(ereg('Safari', $ua_string))
	{
		...
		{	//http://developer.apple.com/internet/safari/uamatrix.html
			...
			if(eregi('Version', $ua_string)) $ua['ua_version'] = get_ua_version($ua_string, 'Version');
			elseif(eregi('Safari/417.8', $ua_string)) $ua['ua_version'] = '2.0.3';
			...
			else $ua['ua_version'] = get_ua_version($ua_string, 'AppleWebKit').' (WebKit)';
	...
	elseif(ereg('PlayStation Portable', $ua_string))
	{
		$ua['name'] = 'PlayStation Portable';
		$ua['ua_class'] = 'PSP';
		$ua['ua_type'] = 'MobileDevice';
	}
	elseif(eregi('PlayStation', $ua_string))
	{
		$ua['name'] = 'PlayStation';
		$ua['ua_class'] = 'PlayStation';
		$ua['ua_type'] = 'Browser';
	}
...
'Program Shareware', 'PerMan Surfer', 'genieBot', 'LetsCrawl.com', 'sohu-search');
	...
	{	//http://msdn.microsoft.com/workshop/author/dhtml/overview/aboutuseragent.asp
		...
		if($detailed)
		{
			...
			elseif(eregi('Windows NT 6.1', $ua_string)) {$ua['os_version'] = 'Seven';$ua['os_class'] = 'WindowsVista';}
	...
	elseif(eregi('Linux',$ua_string))
	{
		...
		if($detailed)
		{
			...
			elseif(eregi('Android', $ua_string)) {$ua['os_version'] = 'Android';$ua['os_class'] = 'Android';}
	...
	elseif(ereg('Mac',$ua_string))
	{
		...
		if($detailed)
		{
			if(eregi('iPhone', $ua_string)) {$ua['os_version'] = 'OS X Mobile (iPhone)';$ua['os_class'] = 'MacOSX';}
			elseif(eregi('iPod', $ua_string)) {$ua['os_version'] = 'OS X Mobile (iPod)';$ua['os_class'] = 'MacOSX';}
			elseif(eregi('Mac OS X', $ua_string)) {$ua['os_version'] = 'OS X';$ua['os_class'] = 'MacOSX';}
